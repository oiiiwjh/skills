...
一个模块化、自包含的技能集合，用于扩展各类编码代理的能力，提供专业化的知识、工作流程和工具集成。

## 概述

本仓库包含可复用的代理技能。技能是针对特定领域或任务的“入职指南”，把领域知识、执行流程和可复用工具组织成可被代理检索和应用的结构。
...
**描述**: 创建有效技能的指南。当用户想要创建新技能（或更新现有技能）以扩展代理的专业知识、工作流程或工具集成能力时，应使用此技能。
...
对于 Codex 以及其他会扫描 `~/.agents/skills` 的代理，直接将仓库克隆到该目录即可：

```bash
git clone <repo-url> ~/.agents/skills
```

对于 OpenCode，建议仍然将本仓库保留在 `~/.agents/skills` 作为技能源目录，再把技能软链接到 `~/.config/opencode/skill`：
...
### 简洁是关键
上下文窗口是公共资源。只添加代理尚未拥有的上下文。质疑每条信息：“这段解释真的有必要吗？”
...
*本仓库为编码代理提供专业化技能。创建或修改技能时，请遵循现有技能中建立的模式和约定。*
